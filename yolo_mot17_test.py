...
image_files = sorted([f for f in os.listdir(mot17_folder) if f.endswith(('.jpg', '.png', '.jpeg'))])

# === Process each image === #
for idx, image_file in enumerate(image_files[:10]):  # Process first 10 images by default
    image_path = os.path.join(mot17_folder, image_file)
    ...
    cv2.imshow("YOLO MOT17 Detection", annotated_frame)
